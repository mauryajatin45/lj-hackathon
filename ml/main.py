...
def _ext_from_mime(mime: Optional[str], allowed: set[str]) -> Optional[str]:
    if not mime: return None
    guess = mimetypes.guess_extension(mime)
    return guess.lower() if (guess and guess.lower() in allowed) else None
...
async def detect_deepfake_audio(
    ...
    if not file_url:
        raise HTTPException(status_code=400, detail="Provide 'file_url'")

    tmp_path = None
    filename = None
    t0 = time.time()
    try:
        file_bytes = await anyio.to_thread.run_sync(s3_download_file, file_url)
        if not file_bytes:
            raise HTTPException(status_code=404, detail="File not found or could not be downloaded")
        url_name = os.path.basename(file_url.split("?")[0].split("#")[0])
        filename = url_name or "remote_audio"
        ext = _safe_ext_from_name(filename, ALLOWED_AUDIO_EXTS, ".wav")
        ...
        async with DF_SEM:
            start = time.time()
            is_deepfake, confidence = await anyio.to_thread.run_sync(
                deepfake_audio_detector, tmp_path
            )
            elapsed = time.time() - start

        explanation = ["High confidence deepfake detection."] if confidence > 0.7 else ["Moderate confidence deepfake detection."] if confidence > 0.5 else ["Low confidence detection."]
        return {
            "is_deepfake": bool(is_deepfake),
            "confidence": float(confidence),
            "prediction": "synthetic" if is_deepfake else "real",
...
@app.post("/detect_deepfake", response_model=DeepfakeResponse)
async def detect_deepfake(
    ...
    name = os.path.basename(file_url).lower()
    if any(name.endswith(ext) for ext in ALLOWED_AUDIO_EXTS):
        return await detect_deepfake_audio(file_url=file_url)
    if any(name.endswith(ext) for ext in ALLOWED_VIDEO_EXTS):
        return await detect_deepfake_video(file_url=file_url)
    raise HTTPException(status_code=400, detail="Unable to determine file type from URL. Please specify audio or video explicitly.")
